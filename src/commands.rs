...
pub struct ThreadArgs {
    ...
    /// acknowledges the risk; it does not disable detection for other paths.
...
pub fn cmd_thread(args: &ThreadArgs) -> Result<()> {
    ...
    // and warn on stderr. Skipped when redaction was applied (already scrubbed
    // for valid-UTF-8 bodies) or when the user passed `--i-know`. mbox/maildir
    // are not scanned here (their renderers stream to files/dirs without a
    // single in-memory buffer); the warning is a best-effort heuristic anyway.
    let warn_secrets = |bytes: &[u8]| {
        if do_redact || args.i_know {
            return;
        }
        if let Ok(text) = std::str::from_utf8(bytes) {
            if let Some(w) = secrets::scan(text).warning() {
                eprintln!("{w}");
            }
        }
    };
